...
import { APIAdmin, APIUser } from '@/apis';
...
export const EditAdmin = ({ id }) => {
  ...
  useEffect(() => {
    async function fetchAdmin() {
      setIsLoading(true);
      setDetailAdmin(await APIUser.getUserById(id));
      setIsLoading(false);
    }
  ...
  }, [reset, detailAdmin, isDialogOpen]);
  ...
  return (
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      ...
      <DialogContent onClick={() => setIsDialogOpen(!isDialogOpen)} className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="mb-4 text-primary-100">Edit Admin</DialogTitle>
        </DialogHeader>
        ...
        {isLoading ? (
          ...
          <>
            <form onSubmit={handleSubmit(onSubmit)} id="editAdmin" className="space-y-4">
              <InputField
            ...
            <DialogFooter>
              ...
              <Button isloading={isLoading} form="editAdmin" type="submit">
                Simpan
              </Button>
            </DialogFooter>
          </>
